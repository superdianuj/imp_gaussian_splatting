...
os.system("ns-train bad-gaussians --data dataset  --viewer.quit-on-train-completion True --pipeline.model.camera-optimizer.mode 'linear' nerfstudio-data --eval_mode 'all' --train-split-fraction 1.1")

output_pth=os.listdir('outputs/dataset/bad-gaussians')[0]
config_pth=os.path.join(os.path.join('outputs/dataset/bad-gaussians',output_pth),'config.yml')
